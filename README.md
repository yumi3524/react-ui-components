...
├── src/
│   ├── components/
│   │   └── common/                    # 共通UIコンポーネント（Button, Input, Titleなど）とStorybookストーリー
│   ├── showcase/                      # コンポーネントギャラリー
│   │   ├── components/
│   │   │   ├── ComponentCard/         # カード型表示コンポーネント（Header, Content, Features）
│   │   │   └── TabButton.tsx          # タブボタン
│   │   ├── pages/
│   │   │   ├── Home.tsx               # ホームページ
...
│   │   │   ├── components/            # コンポーネントデータ（カテゴリ、フォーム、表示系）
│   │   │   └── snippets/              # コードスニペット（各コンポーネントのサンプルコード）
│   │   └── types.ts                   # 型定義
│   ├── App.tsx                        # メインアプリケーション
